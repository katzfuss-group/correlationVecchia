...
double GpGp_matern_scaledim_cpp(const arma::rowvec & x1, const arma::rowvec & x2, const arma::rowvec covparms) {
  ...
  arma::rowvec newx1 = x1;
  arma::rowvec newx2 = x2;

  for(int j = 0; j < dim; j++) {

    newx1(j) = newx1(j) / covparms(1 + j);
    newx2(j) = newx2(j) / covparms(1 + j);
  }

  disx = norm(newx1 - newx2);
  if( disx == 0.0 ) {

    cov = covparms(0) + nugget;

  } else {

    cov = normcon * pow( disx, smooth ) * boost::math::cyl_bessel_k(smooth, disx);
  }
...
double GpGp_matern_spacetime_cpp(const arma::rowvec & x1, const arma::rowvec & x2, const arma::rowvec covparms) {
  ...
  int dp1 = x1.n_elem;
  arma::rowvec newparms(dp1 + 3);

  newparms(0) = covparms(0);
  for(int i = 1; i < dp1; i++) {

    newparms(i) = covparms(1);
  }

  newparms(dp1) = covparms(2);
  newparms(dp1 + 1) = covparms(3);
  newparms(dp1 + 2) = covparms(4);

  return( GpGp_matern_scaledim_cpp(x1, x2, newparms) );
}
