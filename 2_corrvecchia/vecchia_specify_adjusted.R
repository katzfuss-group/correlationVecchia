...
#####
##### Description: This is a simply adjusted version of the vecchia_specify function.
#####
##########################################################################

vecchia_specify_adjusted <- function(locs, m = -1, ordering, cond.yz, locs.pred, ordering.pred, pred.cond, 
                                     conditioning, mra.options = NULL, verbose = FALSE, which.coord = NULL) {
  ...
  if(missing(locs.pred)){  # no prediction
    ...
    if(ordering=='coord') { ord=order_coordinate(locs, which.coord)
