...
#' 
#' ## estimate parameters
#' fit=fit_scaled(y,inputs, trend = 'intercept')
#' fit$covparms
#' 
#' fitc <- fit_corrvecchia(y, inputs, trend = 'intercept', covfun = "matern35_scaledim")
#' fitc$covparms
#' 
#' ## generate spacetime data
#' inputs=matrix(runif(300),ncol=3)
#' y=sin(rowSums(inputs*5))
#' 
#' fitc <- fit_corrvecchia(y, inputs, trend = 'intercept', covfun = "matern_spacetime")
#' fitc$covparms
fit_corrvecchia <- function(y, inputs, ms = c(30), trend = 'zero', X, scale = 'parms', covfun, var.ini, ranges.ini, nu = 3.5, nug = 0, select = Inf, print.level = 0, max.it = 32, tol.dec = 4, n.est = min(5e3, nrow(inputs)))
{
  ### dimensions
  n = nrow(inputs)
  d = ncol(inputs)
...
#' Fitting parameters using scaled Vecchia, assuming matern covariance (just copied from scaledVecchia)
#'
