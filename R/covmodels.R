...
###
###   Overview: This script includes R functions to implement covariance functions.
###
###   Contents:
###     cov_expo_iso / cov_expo_aniso / cov_expo_spacetime / cov_expo_spacetime_nonsep / cov_expo_squared
###     cov_matern_2.5 / cov_matern_3.5 / cov_matern_4.5 / cov_matern / cov_matern_iso / cov_matern_aniso / cov_matern_scaledim / cov_matern_spacetime
###     cov_matern_ns_bruteforce / cov_matern_ns
###     cov_wave
...
#' @title Space-time exponential covariance function
#'
#' @description From a location matrix \code{locs} and a vector with covariance parameters \code{covparms}, this function returns a space-time exponential covariance matrix.
#'
#' @param locs A numerical matrix with \code{n} rows and \code{d} columns. Each row of locs gives a point in R^d
#' @param covparms A numerical vector with covariance parameters = (sigma2, spatial range, temporal range)
#'
#' @return A matrix with \code{n} rows and \code{n} columns, with the \code{(i, j)} entry containing the space-time exponenital covariance between observations \code{locs[i, ]} and \code{locs[j, ]}
#'
#' @export
#'
#' @examples
...
#' locs
#'
#' covmat <- cov_expo_spacetime(locs = locs, covparms = c(1, 0.1, 1.0))
#' fields::image.plot(covmat)
#'
#' covmat <- cov_expo_spacetime(locs = locs, covparms = c(1, 0.1, 1.0))
#' fields::image.plot(covmat)
cov_expo_spacetime <- function(locs, covparms)
{
  # checkargs: locs
  if(!is.matrix(locs) & !is.data.frame(locs)) stop("The argument locs is neither matrix nor data.frame.")
  if(!is.numeric(locs)) stop("The argument locs is not numeric.")
  ...
  # return
  return( cov_expo_iso(locs = locs, covparms = c(covparms[1], 1)) )
}

#' @title Non-separable space-time exponential covariance function
#'
#' @description From a location matrix \code{locs} and a vector with covariance parameters \code{covparms}, this function returns a non-separable space-time exponential covariance matrix.
#'
...
#' fields::image.plot(covmat)
cov_expo_spacetime_nonsep <- function(locs, covparms)
{
  # checkargs: locs
  if(!is.matrix(locs) & !is.data.frame(locs)) stop("The argument locs is neither matrix nor data.frame.")
  if(!is.numeric(locs)) stop("The argument locs is not numeric.")
...
#'
#' @description From a location matrix \code{locs} and a vector with covariance parameters \code{covparms}, this function returns a squared exponential covariance matrix.
#'
#' @param locs A numerical matrix with \code{n} rows and \code{d} columns. Each row of locs gives a point in R^d
#' @param covparms A numerical vector with covariance parameters in the form (variance, range)
#'
...
cov_expo_squared <- function(locs, covparms)
{
  # checkargs: locs
  if(!is.matrix(locs) & !is.data.frame(locs)) stop("The argument locs is neither matrix nor data.frame.")
  if(!is.numeric(locs)) stop("The argument locs is not numeric.")
...
#'
#' @description From a location matrix \code{locs} and a vector with covariance parameters \code{covparms}, this function returns an isotropic matern covariance matrix.
#'
#' @param locs A numerical matrix with \code{n} rows and \code{d} columns. Each row of locs gives a point in R^d
...
cov_matern <- function(locs, covparms, tol = .Machine$double.eps)
{
  ...
  if(any(D.scaled < 0)) stop("distance argument must be non-negative!")
...
#' @title Isotropic Matern covariance function (identical with \code{cov_matern})
#'
#' @description From a location matrix \code{locs} and a vector with covariance parameters \code{covparms}, this function returns an isotropic matern covariance matrix.
#'
#' @param locs A numerical matrix with \code{n} rows and \code{d} columns. Each row of locs gives a point in R^d
...
#' @title Anisotropic Matern covariance function
#'
#' @description From a location matrix \code{locs} and a vector with covariance parameters \code{covparms}, this function returns an anisotropic matern covariance matrix.
#'
#' @param locs A numerical matrix with \code{n} rows and \code{d} columns. Each row of locs gives a point in R^d
#' @param covparms A numerical vector with covariance parameters. It can be of two different forms: One is the 4-dim'l vector (variance, range, smoothness, degree of anisotropy) and the other is simply 3-dim'l vector (variance, range, smoothness). In the latter case, a vector of anisotropy must be specified
#' @param a A vector of anisotropy. At \code{NULL} by default
#' @param tol A numerical tolerance for the damped sine function. At \code{.Machine$double.eps} by default
#'
...
#' cov_matern_scaledim(locs, covparms = c(1, 1/5, 1, 0.5))
cov_matern_aniso <- function(locs, covparms, a = NULL, tol = .Machine$double.eps)
{
  # checkargs: locs
  if(!is.matrix(locs) & !is.data.frame(locs)) stop("The argument locs is neither matrix nor data.frame.")
  if(!is.numeric(locs)) stop("The argument locs is not numeric.")
  ...
  # return
  if(length(covparms) == 4 & is.null(a)) {

    locs[, 1] <- locs[, 1] * covparms[4]

    return( cov_matern(locs = locs, covparms = covparms[1:3], tol = tol) )

  } else if(length(covparms) == 3 & is.numeric(a)) {

    return( cov_matern(locs = t(a * t(locs)), covparms = covparms[1:3], tol = tol) )
...
#' cov_matern_aniso(locs, covparms = c(1, 1, 0.5, 5))
#' cov_matern_scaledim(locs, covparms = c(1, 1/5, 1, 0.5))
cov_matern_scaledim <- function(locs, covparms, tol = .Machine$double.eps)
{
  # checkargs: locs
  if(!is.matrix(locs) & !is.data.frame(locs)) stop("The argument locs is neither matrix nor data.frame.")
  if(!is.numeric(locs)) stop("The argument locs is not numeric.")
  ...
  # checkargs: covparms
  if(!is.numeric(covparms)) stop("The argument covparms is not numeric.")
  if(length(covparms) != ncol(locs) + 2) stop("The argument covparms must be a numeric vector of length of ncol(locs) + 2.")

  # calculation
  sigma2 <- covparms[1]
  a <- 1 / covparms[seq(ncol(locs)) + 1]
  nu <- covparms[ncol(locs) + 2]

  # return
  return( cov_matern_aniso(locs = locs, covparms = c(sigma2, 1, nu), a = a, tol = tol) )
}

#' @title Space-time Matern covariance function
#'
...
#'
#' cov_matern_scaledim(locs, covparms = c(1, 0.1, 0.1, 0.3, 0.5))
...
#'
#' @description From a location matrix \code{locs} and functions \code{sigma}, \code{smoothness}, \code{kernel}, this function returns a nonstationary matern covariance matrix.
#'
...
#' @title Nonstationary Matern covariance function
#'
#' @description From a location matrix \code{locs} and functions \code{sigma}, \code{smoothness}, \code{kernel}, this function returns a nonstationary matern covariance matrix.
#'
#' @param locs A numerical matrix with \code{n} rows and \code{p} columns. Each row of locs1 gives a point in R^d
#' @param sigma A numeric function for spatially varying standard deviation \eqn{\sigma( loc )}
#' @param smoothness A numeric function for spatially varying smoothness \eqn{\nu( loc )}
#' @param kernel A matrix-valued function for spatially varying (local) geometric anisotropy \eqn{\Sigma( loc )}
#'
...
cov_wave <- function(locs, covparms, method = "Dampedsine", tol = 1e-8)
{
  ...
  if(method == "Dampedsine") {
    ...
    return(.cov_wave_dampedsine(locs = locs, covparms = covparms, tol = tol))

  } else if(method == "Dampedcosine") {

    return(.cov_wave_dampedcosine(locs = locs, covparms = covparms))

  } else if(method == "BesselJ") {

    return(.cov_wave_besselJ(locs = locs, covparms = covparms))
...
#' @title Isotropic exponential covariance function for bivariate Gaussian Processes using latent dimensions
#'
...
#' @param covparms A numerical vector with covariance parameters. Its form must be (variance, range, latent dimension).
#'
...
cov_latentDim_biv <- function(locs = NULL, locs1 = NULL, locs2 = NULL, covparms)
{
  ### checkargs: locs
  ...
  if(!is.null(locs) & is.null(locs1) & is.null(locs2)) {
    ...
    if(!is.list(locs)) stop("The argument locs must be a list.")

    if(length(locs) != 2) stop("This function is only for bivariate GPs.")
  ...
  if(!is.numeric(locs1)) stop("The first location object is not numeric.")
...
#' @title Isotropic exponential covariance function for trivariate Gaussian Processes using latent dimensions
#'
#' @param locs At \code{NULL} by default. When used, it must be a list of three location matrices which is list(locs1 = locs1, locs2 = locs2, locs3 = locs3)
#' @param locs1 At \code{NULL} by default. When used, it must be a numerical matrix with \code{n1} rows and \code{d} columns. Each row of locs1 gives a point of the first set in R^d
#' @param locs2 At \code{NULL} by default. When used, it must be a numerical matrix with \code{n2} rows and \code{d} columns. Each row of locs1 gives a point of the first set in R^d
#' @param locs3 At \code{NULL} by default. When used, it must be a numerical matrix with \code{n3} rows and \code{d} columns. Each row of locs1 gives a point of the first set in R^d
#' @param covparms A numerical vector with covariance parameters. Its form must be (variance, range, latent dimension).
#'
#' @return An isotropic exponential covariance matrix with \code{n1 + n2 + n3} rows and \code{n1 + n2 + n3} columns
#'
#' @export
#'
#' @examples
...
#'
#' sqrt(sum((crossmat1 - crossmat2)^2))
cov_latentDim_triv <- function(locs = NULL, locs1 = NULL, locs2 = NULL, locs3 = NULL, covparms)
{
  ### checkargs: locs

  # if(is.null(locs) & !is.null(locs1) & !is.null(locs2) & !is.null(locs3)) locs <- list(locs1 = locs1, locs2 = locs2, locs3 = locs3)

  if(!is.null(locs) & is.null(locs1) & is.null(locs2) & is.null(locs3)) {

    if(!is.list(locs)) stop("The argument locs must be a list.")

    if(length(locs) != 3) stop("This function is only for trivariate GPs.")
  ...
  if(!is.matrix(locs3) & !is.data.frame(locs3)) stop("The third location object is neither matrix nor data.frame.")
  if(!is.numeric(locs3)) stop("The third location object is not numeric.")

  ### calculation
  locs1 <- cbind(locs1, 0)
  locs2 <- cbind(locs2, covparms[3])
  locs3 <- cbind(locs3, 2 * covparms[3])
...
#' @title Isotropic exponontial covariance function for multivariate Gaussian Processes using latent dimensions
#'
#' @param locs A list of \code{p} location matrices
#' @param covparms A numerical vector with covariance parameters. Its form may be (variance, range, latent dimension). If latent dimension is not provided, k and latent must be provided.
#' @param k The number of latent dimensions. At 1 by default.
#' @param latent A list of \code{p} latent dimension matrices
#'
#' @return An isotropic exponential covariance matrix with \code{sum(nrows of locs)} rows and \code{sum(nrows of locs)} columns
#'
#' @export
#'
#' @examples
...
#'
#' sqrt(sum((crossmat1 - crossmat2)^2))
cov_latentDim_mulv <- function(locs, covparms, k = 1, latent = NULL)
{
  ### checkargs: locs
  if(!is.list(locs)) stop("The argument locs must be a list.")
  ...
  if(k == 1) {
    ...
    if(is.null(latent)) stop("If k is larger than 1, latent (latent dimensions) must be provided.")

    if(!is.list(latent)) stop("The argument latent must be a list.")

    if(length(locs) != length(latent)) stop("The arguments locs and latent must have the same length.")

    for(i in 1:length(locs)) {
      locs[[i]] <- cbind(locs[[i]], latent[[i]])
    }
...
#'
#' covmat1 <- cov_flexMatern_biv(locs1 = locs1, locs2 = locs2,
#'                               sigma.mat = matrix(c(1, 0.5, 0.5, 1), 2, 2),
#'                               nu.mat = matrix(0.5, 2, 2),
#'                               alpha.mat = matrix(1, 2, 2))
#'
#' covmat2 <- cov_flexMatern_biv(locs = list(locs1 = locs1, locs2 = locs2),
#'                               sigma.mat = matrix(c(1, 0.5, 0.5, 1), 2, 2),
#'                               nu.mat = matrix(0.5, 2, 2),
#'                               alpha.mat = matrix(1, 2, 2))
#'
#' sqrt(sum((covmat1 - covmat2)^2))
cov_flexMatern_biv <- function(locs = NULL, locs1 = NULL, locs2 = NULL, sigma.mat, nu.mat, alpha.mat)
{
  ### checkargs: locs
  if(is.null(locs) & is.null(locs1) & is.null(locs2)) stop("Please specify the location matrix using locs xor locs1&2.")
  if(is.null(locs) & !is.null(locs1) & is.null(locs2)) stop("Please specify the location matrix using locs xor locs1&2.")
...
cov_flexMatern_mulv <- function(locs, sigma.mat = NULL, p = NULL, rho = NULL, nu.mat = NULL, nu.vec = NULL, alpha.mat = NULL, alpha = NULL)
{
  ### check arguments
...
#' identical(covmat_each, covmat_each_bruteforce)
#' identical(covmat, covmat_matched)
cov_GK_mulv_bruteforce <- function(locs, sigma.mat = NULL, p = NULL, rho = NULL, nu.mat = NULL, nu.vec = NULL, alpha.mat = NULL, alpha = NULL)
{
  n <- nrow(locs)
...
#' identical(covmat_each, covmat_each_bruteforce)
#' identical(covmat, covmat_matched)
cov_GK_mulv <- function(locs, sigma.mat = NULL, p = NULL, rho = NULL, nu.mat = NULL, nu.vec = NULL, alpha.mat = NULL, alpha = NULL)
{
  n <- nrow(locs)
...
#' @title Matern covariance function for GP and its derivatives
#'
#' @description If d = 1, this function returns a Matern covariance matrix of (GP, dGP/dx) in 1-dimensional domain. If d = 2, this function returns a Matern covariance matrix of (GP, dGP/dx1, dGP/dx2) in 2-dimensional domain.
#'
#' @param locs A matrix of \code{n} locations
#' @param covparms A numerical vector with covariance parameters = c(sigma2, range, smoothness). Smoothness must be 2.5 or 4.5 for now.
#' @param tol A numerical tolerance at \code{.Machine$double.eps} by default
#'
#' @return A matrix with \code{n*d} rows and \code{n*d} columns
#'
#' @export
#'
#' @examples
...
#' t(M) == M # M is symmetric
#' eigen(M)$values # M is positive definite
cov_matern_deriv <- function(locs, covparms, tol = .Machine$double.eps)
{
  ### checkargs
  ...
  ### return
  if(covparms[3] == 2.5 & d == 1) {

    return( .cov_matern_deriv_2.5_1d(locs = locs, covparms = covparms[1:2], tol = tol) )

  } else if(covparms[3] == 2.5 & d == 2) {

    return( .cov_matern_deriv_2.5_2d(locs = locs, covparms = covparms[1:2], tol = tol) )

  } else if(covparms[3] == 4.5 & d == 1) {

    return( .cov_matern_deriv_4.5_1d(locs = locs, covparms = covparms[1:2], tol = tol) )

  } else if(covparms[3] == 4.5 & d == 2) {

    return( .cov_matern_deriv_4.5_2d(locs = locs, covparms = covparms[1:2], tol = tol) )
...
#' @title Matern correlation function for GP and its derivatives
#'
#' @description If d = 1, this function returns a Matern correlation matrix of (GP, dGP/dx) in 1-dimensional domain. If d = 2, this function returns a Matern correlation matrix of (GP, dGP/dx1, dGP/dx2) in 2-dimensional domain.
#'
#' @param locs A matrix of \code{n} locations
#' @param covparms A numerical vector with covariance parameters = c(sigma2, range, smoothness). Smoothness must be 2.5 or 4.5 for now.
#' @param tol A numerical tolerance at \code{.Machine$double.eps} by default
...
#' tol <- .Machine$double.eps
#'
#' M <- cor_matern_deriv(locs, c(covparms, 2.5), tol)
#'
#' t(M) == M # M is symmetric
#' eigen(M)$values # M is positive definite
#'
#' M <- cor_matern_deriv(locs, c(covparms, 4.5), tol)
#'
#' t(M) == M # M is symmetric
#' eigen(M)$values # M is positive definite
#'
...
#' t(M) == M # M is symmetric
#' eigen(M)$values # M is positive definite
cor_matern_deriv <- function(locs, covparms, tol = .Machine$double.eps)
{
  ### checkargs
  if(is.list(locs)) locs <- as.matrix(locs[[1]])
  ...
  if(covparms[3] == 2.5 & d == 1) {
    ...
    return( .cor_matern_deriv_2.5_1d(locs = locs, covparms = covparms[1:2], tol = tol) )

  } else if(covparms[3] == 2.5 & d == 2) {

    return( .cor_matern_deriv_2.5_2d(locs = locs, covparms = covparms[1:2], tol = tol) )

  } else if(covparms[3] == 4.5 & d == 1) {

    return( .cor_matern_deriv_4.5_1d(locs = locs, covparms = covparms[1:2], tol = tol) )

  } else if(covparms[3] == 4.5 & d == 2) {

    return( .cor_matern_deriv_4.5_2d(locs = locs, covparms = covparms[1:2], tol = tol) )
...
#' @title Squared exponential covariance function for GP and its derivatives
#'
#' @description If d = 1, this function returns a squared exponential covariance matrix of (GP, dGP/dx) in 1-dimensional domain. If d = 2, this function returns a squared exponential covariance matrix of (GP, dGP/dx1, dGP/dx2) in 2-dimensional domain.
#'
#' @param locs A matrix of \code{n} locations
...
#' tol <- .Machine$double.eps
#'
#' M <- cov_sqexpo_deriv(locs, covparms, tol)
#'
#' t(M) == M # M is symmetric
#' eigen(M)$values # M is positive definite
#'
...
#' t(M) == M # M is symmetric
#' eigen(M)$values # M is positive definite
cov_sqexpo_deriv <- function(locs, covparms, tol = .Machine$double.eps)
{
  ### checkargs
  if(is.list(locs)) locs <- as.matrix(locs[[1]])
  ...
  if(d == 1) {
    ...
    return( .cov_sqexpo_deriv_1d(locs = locs, covparms = covparms, tol = tol) )

  } else if(d == 2) {

    return( .cov_sqexpo_deriv_2d(locs = locs, covparms = covparms, tol = tol) )
...
#' @title Squared exponential correlation function for GP and its derivatives
#'
#' @description If d = 1, this function returns a squared exponential correlation matrix of (GP, dGP/dx) in 1-dimensional domain. If d = 2, this function returns a squared exponential correlation matrix of (GP, dGP/dx1, dGP/dx2) in 2-dimensional domain.
#'
#' @param locs A matrix of \code{n} locations
#' @param covparms A numerical vector with covariance parameters = c(sigma2, range).
#' @param tol A numerical tolerance at \code{.Machine$double.eps} by default
...
#' tol <- .Machine$double.eps
#'
#' M <- cor_sqexpo_deriv(locs, covparms, tol)
#'
#' t(M) == M # M is symmetric
#' eigen(M)$values # M is positive definite
#'
...
#' t(M) == M # M is symmetric
#' eigen(M)$values # M is positive definite
cor_sqexpo_deriv <- function(locs, covparms, tol = .Machine$double.eps)
{
  ### checkargs
  if(is.list(locs)) locs <- as.matrix(locs[[1]])
  ...
  if(d == 1) {
    ...
    return( .cor_sqexpo_deriv_1d(locs = locs, covparms = covparms, tol = tol) )

  } else if(d == 2) {

    return( .cor_sqexpo_deriv_2d(locs = locs, covparms = covparms, tol = tol) )
...
.cov_matern_deriv_2.5_1d <- function(locs, covparms, tol = .Machine$double.eps)
{
...
.cor_matern_deriv_2.5_1d <- function(locs, covparms, tol = .Machine$double.eps)
{
  # covariance matrix
  n           <- ifelse(is.list(locs), nrow(locs[[1]]), nrow(locs))
  cormat      <- .cov_matern_deriv_2.5_1d(locs = locs, covparms = covparms, tol = tol)
...
.cov_matern_deriv_2.5_2d <- function(locs, covparms, tol = .Machine$double.eps)
{
  if(is.list(locs)) locs <- locs[[1]]
...
.cor_matern_deriv_2.5_2d <- function(locs, covparms, tol = .Machine$double.eps)
{
  # covariance matrix
  n           <- ifelse(is.list(locs), nrow(locs[[1]]), nrow(locs))
  cormat      <- .cov_matern_deriv_2.5_2d(locs = locs, covparms = covparms, tol = tol)
...
.cov_matern_deriv_4.5_1d <- function(locs, covparms, tol = .Machine$double.eps)
{
  if(is.list(locs)) locs <- locs[[1]]
  ...
  # (1, 1) - block = cov(GP, GP)
  covmat[1:n, 1:n] <- cov_matern_4.5(locs = locs, covparms = covparms)

  # (1, 2) - block = cov(GP, GP')
  covmat[1:n, 1:n + n] <- - .partial_cov_matern_4.5(locs = locs, covparms = covparms, coord = c(1), tol = tol)

  # (2, 1) - block = cov(GP', GP)
  covmat[1:n + n, 1:n] <- .partial_cov_matern_4.5(locs = locs, covparms = covparms, coord = c(1), tol = tol)

  # (2, 2) - block = cov(GP', GP')
  covmat[1:n + n, 1:n + n] <- .double_partial_cov_matern_4.5(locs = locs, covparms = covparms, coord = c(1), tol = tol)

  return(covmat)
}

.cor_matern_deriv_4.5_1d <- function(locs, covparms, tol = .Machine$double.eps)
{
  # covariance matrix
  n           <- ifelse(is.list(locs), nrow(locs[[1]]), nrow(locs))
  cormat      <- .cov_matern_deriv_4.5_1d(locs = locs, covparms = covparms, tol = tol)
...
.cov_matern_deriv_4.5_2d <- function(locs, covparms, tol = .Machine$double.eps)
{
  if(is.list(locs)) locs <- locs[[1]]
  ...
  # (2, 1) - block = cov(dGP/dx, GP)
  covmat[1:n + n, 1:n] <- .partial_cov_matern_4.5(locs, covparms, coord = 1, tol = tol)

  # (3, 1) - block = cov(dGP/dy, GP)
  covmat[1:n + 2*n, 1:n] <- .partial_cov_matern_4.5(locs, covparms, coord = 2, tol = tol)

  # (1, 2) - block = cov(GP, dGP/dx) = t( cov(dGP/dx, GP) )
  covmat[1:n, 1:n + n] <- - covmat[1:n + n, 1:n]

  # (2, 2) - block = cov(dGP/dx, dGP/dx)
  covmat[1:n + n, 1:n + n] <- .double_partial_cov_matern_4.5(locs = locs, covparms = covparms, coord = 1, tol = tol)

  # (3, 2) - block = cov(dGP/dy, dGP/dx)
  covmat[1:n + 2*n, 1:n + n] <- .crossdouble_partial_cov_matern_4.5(locs = locs, covparms = covparms, tol = tol)
  ...
  # (3, 3) - block = cov(dGP/dy, dGP/dy)
  covmat[1:n + 2*n, 1:n + 2*n] <- .double_partial_cov_matern_4.5(locs = locs, covparms = covparms, coord = 2, tol = tol)

  return(covmat)
}

.cor_matern_deriv_4.5_2d <- function(locs, covparms, tol = .Machine$double.eps)
{
  # covariance matrix
  n           <- ifelse(is.list(locs), nrow(locs[[1]]), nrow(locs))
  cormat      <- .cov_matern_deriv_4.5_2d(locs = locs, covparms = covparms, tol = tol)
...
.partial_cov_matern_4.5 <- function(locs, covparms, coord, tol = .Machine$double.eps)
{
  D.scaled  <- fields::rdist(x1 = locs, x2 = NULL) / covparms[2]

  const     <- - 9 * covparms[1] / 7 / covparms[2]^2
  xterm     <- matrix(locs[, coord], nrow = nrow(locs), ncol = nrow(locs), byrow = FALSE) - matrix(locs[, coord], nrow = nrow(locs), ncol = nrow(locs), byrow = TRUE)
  polyd     <- 1 + 3 * D.scaled + 18 / 5 * D.scaled^2 + 9 / 5 * D.scaled^3

  return( const * xterm * polyd * exp(- 3 * D.scaled) )
}

.double_partial_cov_matern_4.5 <- function(locs, covparms, coord, tol = .Machine$double.eps)
{
  D.scaled  <- fields::rdist(x1 = locs, x2 = NULL) / covparms[2]

  const     <- 9 * covparms[1] / 7 / covparms[2]^2
  xterm     <- matrix(locs[, coord], nrow = nrow(locs), ncol = nrow(locs), byrow = FALSE) - matrix(locs[, coord], nrow = nrow(locs), ncol = nrow(locs), byrow = TRUE)
  polyp     <- 1 + 3 * D.scaled + 18 / 5 * D.scaled^2 + 18 / 5 * D.scaled^3
  polyq     <- 1 + 3 * D.scaled + 3 * D.scaled^2

  return( const * (polyp - 9 / 5 * (xterm / covparms[2])^2 * polyq ) * exp(- 3 * D.scaled) )
}

.crossdouble_partial_cov_matern_4.5 <- function(locs, covparms, tol = .Machine$double.eps)
{
  D.scaled  <- fields::rdist(x1 = locs, x2 = NULL) / covparms[2]

  const     <- - 81 * covparms[1] / 35 / covparms[2]^4
  xterm     <- matrix(locs[, 1], nrow = nrow(locs), ncol = nrow(locs), byrow = FALSE) - matrix(locs[, 1], nrow = nrow(locs), ncol = nrow(locs), byrow = TRUE)
  yterm     <- matrix(locs[, 2], nrow = nrow(locs), ncol = nrow(locs), byrow = FALSE) - matrix(locs[, 2], nrow = nrow(locs), ncol = nrow(locs), byrow = TRUE)
  polyq     <- 1 + 3 * D.scaled + 3 * D.scaled^2

  return( const * xterm * yterm * polyq * exp(- sqrt(5) * D.scaled) )
}

.cov_sqexpo_deriv_1d <- function(locs, covparms, tol = .Machine$double.eps)
{
  if(is.list(locs)) locs <- locs[[1]]
  ...
  # (1, 1) - block = cov(GP, GP)
  covmat[1:n, 1:n] <- cov_expo_squared(locs = locs, covparms = covparms)

  # (1, 2) - block = cov(GP, GP')
  covmat[1:n, 1:n + n] <- - .partial_cov_sqexpo(locs = locs, covparms = covparms, coord = c(1), tol = tol)

  # (2, 1) - block = cov(GP', GP)
  covmat[1:n + n, 1:n] <- .partial_cov_sqexpo(locs = locs, covparms = covparms, coord = c(1), tol = tol)

  # (2, 2) - block = cov(GP', GP')
  covmat[1:n + n, 1:n + n] <- .double_partial_cov_sqexpo(locs = locs, covparms = covparms, coord = c(1), tol = tol)

  return(covmat)
}

.cor_sqexpo_deriv_1d <- function(locs, covparms, tol = .Machine$double.eps)
{
  # covariance matrix
  n           <- ifelse(is.list(locs), nrow(locs[[1]]), nrow(locs))
  cormat      <- .cov_sqexpo_deriv_1d(locs = locs, covparms = covparms, tol = tol)
...
.cov_sqexpo_deriv_2d <- function(locs, covparms, tol = .Machine$double.eps)
{
  if(is.list(locs)) locs <- locs[[1]]
  ...
  # (2, 1) - block = cov(dGP/dx, GP)
  covmat[1:n + n, 1:n] <- .partial_cov_sqexpo(locs, covparms, coord = 1, tol = tol)

  # (3, 1) - block = cov(dGP/dy, GP)
  covmat[1:n + 2*n, 1:n] <- .partial_cov_sqexpo(locs, covparms, coord = 2, tol = tol)

  # (1, 2) - block = cov(GP, dGP/dx) = t( cov(dGP/dx, GP) )
  covmat[1:n, 1:n + n] <- - covmat[1:n + n, 1:n]

  # (2, 2) - block = cov(dGP/dx, dGP/dx)
  covmat[1:n + n, 1:n + n] <- .double_partial_cov_sqexpo(locs = locs, covparms = covparms, coord = 1, tol = tol)

  # (3, 2) - block = cov(dGP/dy, dGP/dx)
  covmat[1:n + 2*n, 1:n + n] <- .crossdouble_partial_cov_sqexpo(locs = locs, covparms = covparms, tol = tol)
  ...
  # (3, 3) - block = cov(dGP/dy, dGP/dy)
  covmat[1:n + 2*n, 1:n + 2*n] <- .double_partial_cov_sqexpo(locs = locs, covparms = covparms, coord = 2, tol = tol)

  return(covmat)
}

.cor_sqexpo_deriv_2d <- function(locs, covparms, tol = .Machine$double.eps)
{
  # covariance matrix
  n           <- ifelse(is.list(locs), nrow(locs[[1]]), nrow(locs))
  cormat      <- .cov_sqexpo_deriv_2d(locs = locs, covparms = covparms, tol = tol)
...
.partial_cov_sqexpo <- function(locs, covparms, coord, tol = .Machine$double.eps)
{
  D.scaled  <- fields::rdist(x1 = locs, x2 = NULL) / covparms[2]

  const     <- - covparms[1] / covparms[2]^2
  xterm     <- matrix(locs[, coord], nrow = nrow(locs), ncol = nrow(locs), byrow = FALSE) - matrix(locs[, coord], nrow = nrow(locs), ncol = nrow(locs), byrow = TRUE)

  return( const * xterm * exp(- 0.5 * D.scaled^2) )
}

.double_partial_cov_sqexpo <- function(locs, covparms, coord, tol = .Machine$double.eps)
{
  D.scaled  <- fields::rdist(x1 = locs, x2 = NULL) / covparms[2]
  ...
  return( const * (1 - (xterm / covparms[2])^2) * exp(- 0.5 * D.scaled^2) )
}

.crossdouble_partial_cov_sqexpo <- function(locs, covparms, tol = .Machine$double.eps)
{
  D.scaled  <- fields::rdist(x1 = locs, x2 = NULL) / covparms[2]

  const     <- - covparms[1] / covparms[2]^4
  xterm     <- matrix(locs[, 1], nrow = nrow(locs), ncol = nrow(locs), byrow = FALSE) - matrix(locs[, 1], nrow = nrow(locs), ncol = nrow(locs), byrow = TRUE)
  yterm     <- matrix(locs[, 2], nrow = nrow(locs), ncol = nrow(locs), byrow = FALSE) - matrix(locs[, 2], nrow = nrow(locs), ncol = nrow(locs), byrow = TRUE)
