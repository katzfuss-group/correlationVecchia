...
###     generate_hnm / covmat_hnm / cormat_hnm
###
####################################################################################
...
#' @title Generating a realization of hierarchical normal model (non-Euclidean example)
#'
#' @param mu0 Mean of the root normal model
#' @param sig20 Variance of the root normal model
#' @param k Depth of the hierarchical normal model
#' @param sig2s Numeric vector of length of k. The i-th entry is the variance of the normal models in level i. As \code{NULL} (rep(1, k)) by default.
#' @param n Number of leaves. \code{NULL} (mlt^k) as default.
#' @param mlt Number of children for each node
#'
#' @return \code{generate_hnm} returns
#'     \itemize{
...
generate_hnm <- function(mu0, sig20, k, sig2s = NULL, n = NULL, mlt = 2)
{
  ### inputs
  if( is.null(sig2s) ) sig2s <- rep(1, k)

  if( is.null(n) ) n <- mlt^k
  ...
  for( i in 1:k ) {
    ...
    if( i != 1 ) {
      ...
      mus <- muvec
      for( j in seq(mlt * length(mus)) ) {

        muvec[j] <- rnorm( n = 1, mean = mus[(j+mlt-1) %/% mlt], sd = sqrt(sig2s[i-1]) )
      }

    } else {

      muvec[i] <- rnorm( n = 1, mean = mu0, sd = sqrt(sig20) )
    }

    mulst[[i]] <- muvec

  } ; names(mulst) <- paste0("level_", seq(k))

  ### generate y
  ys <- c()
  for( i in 1:n ) {

    ys[i] <- rnorm( n = 1, mean = mulst[[k]][(i+mlt-1) %/% mlt], sd = sqrt(sig2s[k]) )
  }

  ### return
  return(list(mu = mulst, y = ys, mu0 = mu0, sig20 = sig20, k = k, sig2s = sig2s, n = n, mlt = mlt))
}

#' @title Covariance matrix of Hierarchical Normal Model (HNM)
#'
#' @param output output of \code{generate_hnm()}
#'
...
#'
#' fields::imagePlot(cormat)
covmat_hnm <- function(output)
{
  covmat <- matrix(output$sig20, nrow = output$n, ncol = output$n)

  blockize <- function(x,n) split(x, cut(seq_along(x), n, labels = FALSE))

  for(i in 1:output$k) {

    idx <- blockize(seq(output$n), output$mlt^i)

    if(length(idx) != output$n) {

      for(j in 1:length(idx)) covmat[idx[[j]], idx[[j]]] <- covmat[idx[[j]], idx[[j]]] + output$sig2s[i]

    } else {

      covmat <- covmat + diag(output$sig2s[output$k], output$n)
    }
...
#' @title Correlation matrix of Hierarchical Normal Model (HNM)
#'
#' @param output output of \code{generate_hnm()}
#'
...
#'
#' fields::imagePlot(cormat)
cormat_hnm <- function(output)
{
  covmat    <- covmat_hnm(output)
  n         <- nrow(covmat)
  diagvec   <- diag(covmat)

  covmat    <- covmat / sqrt( matrix(diagvec, n, n, byrow = TRUE) * matrix(diagvec, n, n, byrow = FALSE) )
