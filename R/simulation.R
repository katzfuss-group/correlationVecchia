...
#'                                          method.locs = 'random', corr.dist = "1-rho", 
#'                                          method.modify = NULL, 
#'                                          pivot = FALSE, tol = .Machine$double.eps, 
#'                                          verbose = TRUE, 
#'                                          covmodel = cov_expo_aniso, covparms = c(1, 0.1, 10))
#' out$kls.average
simulate_univariate_knownCovparms <- function(nsim, n, d, m, method.locs, method.modify = NULL, corr.dist = "1-rho", pivot = FALSE, tol = .Machine$double.eps, verbose = TRUE, covmodel, ...)
{
  time.tot      <- proc.time()
  time.sim      <- list()
  time.gen      <- proc.time()
  ...
  for(k in 1:nsim) {
    ...
    approx[[1]]   <- corrvecchia_specify_knownCovparms_2(locs = locs, m = m, ordering = "maxmin", ordering.method = "euclidean", coordinate = NULL, corr.dist = corr.dist, initial.pt = NULL, conditioning = "NN", conditioning.method = "euclidean", covmodel = covmat.modified, covparms = c(1))
    if(verbose == TRUE) message(paste0("System: E-Maxmin + E-NN Vecchia approximation is accomplished. [", Sys.time(), "]"))
    approx[[2]]   <- corrvecchia_specify_knownCovparms_2(locs = locs, m = m, ordering = "maxmin", ordering.method = "euclidean", coordinate = NULL, corr.dist = corr.dist, initial.pt = NULL, conditioning = "NN", conditioning.method = "correlation", covmodel = covmat.modified, covparms = c(1))
    if(verbose == TRUE) message(paste0("System: E-Maxmin + C-NN Vecchia approximation is accomplished. [", Sys.time(), "]"))
    approx[[3]]   <- corrvecchia_specify_knownCovparms_2(locs = locs, m = m, ordering = "maxmin", ordering.method = "correlation", coordinate = NULL, corr.dist = corr.dist, initial.pt = NULL, conditioning = "NN", conditioning.method = "euclidean", covmodel = covmat.modified, covparms = c(1))
    if(verbose == TRUE) message(paste0("System: C-Maxmin + E-NN Vecchia approximation is accomplished. [", Sys.time(), "]"))
    approx[[4]]   <- corrvecchia_specify_knownCovparms_2(locs = locs, m = m, ordering = "maxmin", ordering.method = "correlation", coordinate = NULL, corr.dist = corr.dist, initial.pt = NULL, conditioning = "NN", conditioning.method = "correlation", covmodel = covmat.modified, covparms = c(1))
    if(verbose == TRUE) message(paste0("System: C-Maxmin + C-NN Vecchia approximation is accomplished. [", Sys.time(), "]"))
    
    approx[[5]]   <- corrvecchia_specify_knownCovparms_2(locs = locs, m = m, ordering = "coord", ordering.method = "euclidean", coordinate = c(1), corr.dist = corr.dist, initial.pt = NULL, conditioning = "NN", conditioning.method = "euclidean", covmodel = covmat.modified, covparms = c(1))
    if(verbose == TRUE) message(paste0("System: X-coord + E-NN Vecchia approximation is accomplished. [", Sys.time(), "]"))
    approx[[6]]   <- corrvecchia_specify_knownCovparms_2(locs = locs, m = m, ordering = "coord", ordering.method = "euclidean", coordinate = c(2), corr.dist = corr.dist, initial.pt = NULL, conditioning = "NN", conditioning.method = "euclidean", covmodel = covmat.modified, covparms = c(1))
...
#'                                            method.locs = 'random', 
#'                                            method.modify = NULL, corr.dist = "1-rho",
#'                                            pivot = FALSE, tol = .Machine$double.eps,
#'                                            verbose = TRUE,
#'                                            covmodel = cov_bivariate_expo_latDim, 
#'                                            covparms = c(1, 0.1, 0.1))
#' out$kls.average
simulate_multivariate_knownCovparms <- function(nsim, n, d, p, m, method.locs, method.modify = NULL, corr.dist = "1-rho", pivot = FALSE, tol = .Machine$double.eps, verbose = TRUE, covmodel, ...)
{
  time.tot      <- proc.time()
  time.sim      <- list()
  time.gen      <- proc.time()
  ...
  for(k in 1:nsim) {
    ...
    approx[[4]]   <- baseline_4_multivariate_specify(locs = realization$sim[[k]]$locs, m = m, corr.dist = corr.dist, covmodel = covmodel, ...)
    if(verbose == TRUE) message(paste0("System: The fourth baseline approximation is accomplished. [", Sys.time(), "]"))
    approx[[5]]   <- corrvecchia_specify_knownCovparms_2(locs = locs, m = m, ordering = "maxmin", ordering.method = "correlation", coordinate = NULL, corr.dist = corr.dist, initial.pt = NULL, conditioning = "NN", conditioning.method = "correlation", covmodel = covmat.modified, covparms = c(1))
    if(verbose == TRUE) message(paste0("System: C-Maxmin + C-NN Vecchia approximation is accomplished. [", Sys.time(), "]"))
...
#' out$kls.average
simulate_spacetime_knownCovparms <- function(nsim, n, d, t.len, m, method.locs, method.modify = NULL, corr.dist = "1-rho", pivot = FALSE, tol = .Machine$double.eps, verbose = TRUE, covmodel, ...)
{
  time.tot      <- proc.time()
  time.sim      <- list()
  time.gen      <- proc.time()
...
#'
#' @examples
#' out <- simulate_derivative_knownCovparms(nsim = 1, n = 10^2, d = 1, m = 10, 
#'                                          method.locs = 'random', corr.dist = "1-rho", 
#'                                          covmodel = corr_derivative_squared_expo_1d, 
#'                                          covparms = c(1, 0.1), pivot = FALSE, 
#'                                          method.modify = "eigen-I", tol = 1e-4)
#' out$kls.average
#' 
#' \dontrun{
...
#' out$kls.average
#' }
simulate_derivative_knownCovparms <- function(nsim, n, d, m, method.locs, method.modify = NULL, corr.dist = "1-rho", pivot = FALSE, tol = .Machine$double.eps, verbose = TRUE, covmodel, ...)
{
  time.tot      <- proc.time()
  time.sim      <- list()
  time.gen      <- proc.time()
  ...
  for(k in 1:nsim) {
    ...
    if(verbose == TRUE) message(paste0("System: The third baseline approximation is accomplished. [", Sys.time(), "]"))
    approx[[4]] <- baseline_4_multivariate_specify(locs = locs, m = m, corr.dist = corr.dist, covmodel = covmodel, ...)
    if(verbose == TRUE) message(paste0("System: The fourth baseline approximation is accomplished. [", Sys.time(), "]"))
    approx[[5]] <- corrvecchia_specify_knownCovparms_2(locs = locs.all, m = m, ordering = "maxmin", ordering.method = "correlation", coordinate = NULL, corr.dist = corr.dist, initial.pt = NULL, conditioning = "NN", conditioning.method = "correlation", covmodel = covmat.modified, covparms = c(1))
    if(verbose == TRUE) message(paste0("System: C-Maxmin + C-NN Vecchia approximation is accomplished. [", Sys.time(), "]"))
