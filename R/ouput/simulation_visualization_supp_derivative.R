...
out     <- output.deriv.matern25.2d
vdat3   <- out$vars %>% left_join(out$kldiv, by = "index") %>% filter(r == 0.1) %>% select(-r)
vdat4   <- out$vars %>% left_join(out$kldiv, by = "index") %>% filter(m == 20) %>% select(-m)

vdat1      <- vdat1 %>% select(-index) %>% tidyr::gather(key = "approx", value = "kldiv", -m)
vdat2      <- vdat2 %>% select(-index) %>% tidyr::gather(key = "approx", value = "kldiv", -r)
vdat3      <- vdat3 %>% select(-index) %>% tidyr::gather(key = "approx", value = "kldiv", -m)
vdat4      <- vdat4 %>% select(-index) %>% tidyr::gather(key = "approx", value = "kldiv", -r)
...
ftn         <- log10
xlabs       <- c("increasing m", "increasing r", "increasing m", "increasing r")
...
p2 <- vdat2 %>% ggplot(aes(x = r, y = ftn(kldiv), col = approx, shape = approx)) +
  geom_point(size = size.point) + geom_line(size = size.line, alpha = alpha) +
  scale_color_manual(values = cols, labels = legs, guide = guide_legend(nrow = 1, byrow = TRUE)) +
  scale_shape_manual(values = shps, labels = legs, guide = guide_legend(nrow = 1, byrow = TRUE)) +
...
p4 <- vdat4 %>% ggplot(aes(x = r, y = ftn(kldiv), col = approx, shape = approx)) +
  geom_point(size = size.point) + geom_line(size = size.line, alpha = alpha) +
  scale_color_manual(values = cols, labels = legs, guide = guide_legend(nrow = 1, byrow = TRUE)) +
  scale_shape_manual(values = shps, labels = legs, guide = guide_legend(nrow = 1, byrow = TRUE)) +
...
ggplot2::ggsave("simulation_supp_deriv_matern25.pdf", result, width = 15.2, height = 10)
...
ggplot2::ggsave("simulation_supp_deriv_matern45.pdf", result, width = 15.2, height = 10)
...
ggplot2::ggsave("simulation_supp_deriv_sqexpo.pdf", result, width = 15.2, height = 10)
